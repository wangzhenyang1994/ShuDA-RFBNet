...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 29 15:22:13 2018
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        image, labels, label_img = sample['image'], sample['label'], sample['seg_label']
...
#        filled_labels[range(len(labels))[:self.max_objects]] = labels[:self.max_objects]
        return {'image': torch.from_numpy(image), 'label': labels, 'seg_label': torch.from_numpy(label_img).type(torch.LongTensor)}
...
class ResizeImage(object):
    ...
    def __call__(self, sample):
        image, label, label_img = sample['image'], sample['label'], sample['seg_label']
        image = cv2.resize(image, self.new_size, interpolation=self.interpolation)
        label_img = cv2.resize(label_img, self.new_size, interpolation=self.interpolation)
        return {'image': image, 'label': label, 'seg_label': label_img}
